...
@section('header')
    <x-page-header title="言葉を編集する" :backUrl="route('dictionaries.words.show',[$dictionary, $word])" />
@endsection
...
@section('content')
    ...
    <form id="wordForm" action="{{ route('dictionaries.words.update',[$dictionary, $word]) }}" method="POST" enctype="multipart/form-data">   
        ...
        <div class="rounded-xl mt-4 px-4 py-4 mb-4" style="background:#fff; border:1px solid #E0D4C0;">
            ...
            <input type="text" name="headword" id="headword"
                value="{{ old('headword',$word->headword) }}"
            ...
            <input type="text" name="reading" id="reading"
                ...
                style="background:#F6F2EC; border:1px solid #E0D4C0;">
            <input type="hidden" name="initial_char" id="initial_char" value="{{ old('initial_char',$word->initial_char) }}">
        </div>
        ...
        <div class="rounded-xl px-4 py-4 mb-4" style="background:#fff; border:1px solid #E0D4C0;">
            ...
            <textarea name="raw_episode" id="raw_episode" rows="6"
                    ...
                    style="background:#F6F2EC; border:1px solid #E0D4C0;">{{ old('raw_episode',$word->raw_episode) }}</textarea>
        </div>
    ...
    {{-- 危険な操作 --}}
    <div class="mb-8">
        <form action="{{ route('dictionaries.words.destroy', [$dictionary, $word]) }}" method="POST"
            onsubmit="return confirm('「{{ $word->headword }}」を削除しますか？この操作は取り消せません。')">
            @csrf
            @method('DELETE')
            <button type="submit"
                    class="w-full rounded-xl py-3.5 text-sm font-bold flex items-center justify-center gap-2"
    ...
    <script>
        ...
        document.getElementById('reading').addEventListener('input', function() {
            ...
            const rowMap = {
                'あ': ['あ','い','う','え','お'],'か': ['か','き','く','け','こ','が','ぎ','ぐ','げ','ご'],
                'さ': ['さ','し','す','せ','そ','ざ','じ','ず','ぜ','ぞ'],'た': ['た','ち','つ','て','と','だ','ぢ','づ','で','ど'],
                'な': ['な','に','ぬ','ね','の'],'は': ['は','ひ','ふ','へ','ほ','ば','び','ぶ','べ','ぼ','ぱ','ぴ','ぷ','ぺ','ぽ'],
                'ま': ['ま','み','む','め','も'],'や': ['や','ゆ','よ'],'ら': ['ら','り','る','れ','ろ'],'わ': ['わ','を','ん'],
            };
            let initial = first;
        ...
        function updateAiBtn() {
            const hasEpisode = document.getElementById('raw_episode').value.trim();
            const hasHeadword = document.getElementById('headword').value.trim();
            const btn = document.getElementById('aiBtn');
            if (!btn) return;

            if (hasEpisode && hasHeadword) {
                btn.style.background = '#E8A030';
                btn.style.color = '#fff';
                btn.disabled = false;
            } else {
                btn.style.background = '#F2E8D8';
                btn.style.color = '#C8A878';
                btn.disabled = true;
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Laravelから渡されるデータを確認
            // キャストされていない場合を考慮して、念のためパースを試みる
            let existingData = @json($word->dictionary_data);
            
            // 文字列で届いている場合はJSONとしてパース
